...
pub contract interface NonFungibleToken {
    ...
    pub resource interface Provider {
        pub fun withdraw(withdrawID: UInt64): @NFT {
            post {
                result.id == withdrawID: "The withdrawn token's identification must align with the specified requested ID."
            }
        }
    }
    ...
    pub fun createEmptyCollection(): @Collection {
        post {
            result.getIDs().length == 0: "The newly generated collection should have no items in it.!"
        }
    }
}